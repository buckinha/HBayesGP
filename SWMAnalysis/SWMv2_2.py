"""SWM, A Simple Wildfire-inspired MDP model. Version 2.2"""
...
-Discounting
-Option for ignoring a certain number of timesteps at the beginning of the simulation to let things equalize
...
def simulate(timesteps, policy=[0,0,0,0,0,0], random_seed=0, steps_to_ignore=0, model_parameters={}, SILENT=True, PROBABILISTIC_CHOICES=True):
    ...
    if steps_to_ignore < 0: steps_to_ignore = 0
    ...
    #discounting
    discount_rate = 0.97
    if "Discount Rate" in model_parameters.keys(): supp_cost_mild = model_parameters["Discount Rate"]    
    ...
    for i in range((-1 * steps_to_ignore), timesteps):
        ...
        #check to see if we've gotten past the "ignored" timesteps
        if i >= 0: 
            #we're done ignoring any initial timesteps
            states[i] = {
    ...
    vals = [0.0] * timesteps
    vals_disc = [0.0] * timesteps
    hab = [0.0] * timesteps
    suppressions = 0.0
    joint_prob = 1.0
    prob_sum = 0.0

    net_discount = 1.0 / discount_rate
    ...
    for i in range(timesteps):
        ...
        vals[i] = states[i]["Reward"]
        #calculate this timestep's discount rate
        net_discount *= discount_rate
        vals_disc[i] = states[i]["Reward"] * net_discount
        hab[i] = states[i]["Habitat"]
    ave_prob = prob_sum / timesteps

    summary = {
                "Average State Value": round(np.mean(vals),3),
                "Total Pathway Value": round(np.sum(vals),3),
                "Discounted Value": round(np.sum(vals_disc),3),
